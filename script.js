...
function loadGame() {
    ...
    if (saved) {
        ...
        // 風フロア状態の復元
        isWindFloor = data.isWindFloor || false;
        windTimer = 0;

        // マルチスクリーン状態の復元
        multiScreenMode = data.multiScreenMode || false;
        screenGridSize = data.screenGridSize || 2;
...
function saveGame() {
    ...
    const data = {
        ...
        // 風フロア情報
        isWindFloor: isWindFloor,

        // マルチスクリーン情報
        multiScreenMode: multiScreenMode,
        screenGridSize: multiScreenMode ? screenGridSize : null,
...
function initMap() {
    ...
    if (floorLevel >= 50 && floorLevel < 100 && !fixedStageFloors.includes(floorLevel)) {
        ...
        for (let sy = 0; sy < screenGridSize; sy++) {
            for (let sx = 0; sx < screenGridSize; sx++) {
                screenGrid.wind[sy][sx] = Math.random() < 0.03;
            }
        }
        // スタート画面(0,0)は風なし
        ...
        // セーブポイント: 中間の画面に配置
        const saveSY = Math.floor(screenGridSize / 2);
        const saveSX = screenGridSize > 2 ? Math.floor(screenGridSize / 2) : 0;
        const saveMap = screenGrid.maps[saveSY][saveSX];
        for (let tries = 0; tries < 50; tries++) {
    ...
    let isGreatHallFloor = !isDenseMazeFloor && !isSpiralFloor && !isCrossFloor && !isIslandsFloor && !isMazeFloor && layoutRoll < 0.52;
    ...
    // フロアタイプに応じて部屋数を決定
    const roomCount = isDenseMazeFloor ? 8 : (isSpiralFloor ? 6 : (isCrossFloor ? 6 : (isIslandsFloor ? 12 : (isMazeFloor ? 25 : (isGreatHallFloor ? 2 : (Math.floor(Math.random() * 4) + 8))))));
    ...
    const extraConnCount = isDenseMazeFloor ? 25 : (isSpiralFloor ? 5 : (isIslandsFloor ? 2 : (isMazeFloor ? 20 : 3)));
    ...
    if (floorLevel >= 36 && !isWindFloor && !fixedStages.includes(floorLevel) && Math.random() < 0.03) {
        isWindFloor = true;
    ...
    if (!multiScreenMode && isMerchantFloor) {
        ...
        for (const srcRoom of srcRooms) {
            ...
            for (const dir of dirs) {
                ...
                // 通路と反対側の隅に商人を配置
                let mcx, mcy;
                if (dir.dx === 1) { mcx = sx + shopW - 1; mcy = sy; }
                else if (dir.dx === -1) { mcx = sx; mcy = sy; }
    ...
    // --- 1部屋あたりの魔導書を最大2個に制限 ---
    const tomeSymbols = [SYMBOLS.SPEED, SYMBOLS.CHARM, SYMBOLS.STEALTH, SYMBOLS.HEAL_TOME, SYMBOLS.EXPLOSION, SYMBOLS.ESCAPE, SYMBOLS.BREAKER_TOME];
    rooms.forEach(room => {
        let tomeCount = 0;
        for (let y = room.y; y < room.y + room.h; y++) {
            for (let x = room.x; x < room.x + room.w; x++) {
...
async function startFloorTransition() {
    ...
    // 着地後にBGM開始（100階は無音）
    if (floorLevel !== 100) changeBGMTrack();
    ...
    if (gameState === 'ENDING_SEQ' && transition.active) {
        ...
        for (let y = 0; y < ROWS; y++) {
            for (let x = 0; x < COLS; x++) {
                ...
                if (char === SYMBOLS.WALL) {
                    ...
                    ctx.restore();
                } else if ([SYMBOLS.WAND, SYMBOLS.KEY, SYMBOLS.SWORD, SYMBOLS.ARMOR, SYMBOLS.SPEED, SYMBOLS.CHARM, SYMBOLS.STEALTH, SYMBOLS.HEAL_TOME, SYMBOLS.EXPLOSION, SYMBOLS.ESCAPE, SYMBOLS.BREAKER_TOME].includes(char)) {
                    // 魔導書系アイテムはすべて統一アイコンで表示（拾うまで種類不明）
                    const isTome = [SYMBOLS.SPEED, SYMBOLS.CHARM, SYMBOLS.STEALTH, SYMBOLS.HEAL_TOME, SYMBOLS.EXPLOSION, SYMBOLS.ESCAPE, SYMBOLS.BREAKER_TOME].includes(char);
        ...
        enemies.forEach(e => {
            ...
            if (e.type === 'DRAGON') {
                ...
                else if (e.type === 'LAYER') { eColor = '#a78bfa'; eChar = SYMBOLS.LAYER; }
...
async function dragonWaveAttack(wave = 1) {
    ...
    const toExplode = tempWalls.filter(w => w.type === 'ICICLE' || w.type === 'BLOCK');
    tempWalls = tempWalls.filter(w => w.type !== 'ICICLE' && w.type !== 'BLOCK');
...
async function attackEnemy(enemy, dx, dy, isMain = true) {
    ...
    if (hasRing('KNOCKBACK_RING') && enemy.hp > 0 && enemy.type !== 'TURRET' && enemy.type !== 'SNAKE' && enemy.type !== 'DRAGON' && enemy.type !== 'SUMMONER') {
...
async function enemyTurn() {
    ...
    // 遭難した冒険者（商人）の逃走判定：敵またはウィルが隣接していたら逃げる
...
window.addEventListener('keydown', async e => {
    ...
    if (e.key === 'Enter') {
        ...
        if (gameState === 'TITLE') {
            ...
            else if (menuSelection === 1) { gameState = 'RINGS'; ringEquipSelection = 0; }
