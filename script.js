...
function initMap() {
    ...
    if (floorLevel === 13) {
        ...
        // 毒沼：下の壁から4マスのライン（y = ROWS-5）を全面に。穴付近（右端）は除外
        const poisonLineY = ROWS - 5; // y = 20
        const poisonClearX = COLS - 7; // x >= 33 は除外（穴・タレット付近）
        for (let x = 2; x < poisonClearX; x++) {
            map[poisonLineY][x] = SYMBOLS.POISON;
        }
...
function drawTitle() {
    ...
    const allOptions = [...baseOptions, 'TEST PLAY', 'DEEP TEST'];
...
async function enemyTurn() {
    ...
    for (let i = enemies.length - 1; i >= 0; i--) {
                ...
                if (!playerDetected) {
                    ...
                    if (allies.length > 0) {
                        ...
                        if (bfs.dx !== 0 || bfs.dy !== 0) {
                            ...
                            if (!isRealHole(nx, ny) && canEnemyMove(nx, ny, e) &&
                                !enemies.some(o => !o._dead && o.hp > 0 && o.x === nx && o.y === ny)) {
                                e.x = nx; e.y = ny;
                            }
                        }
                    }
        ...
        if (e.faction && !e.isAlly && e.type !== 'BREAKER') {
            ...
            if (factionTarget) {
                if (factionDist <= 1) {
                    ...
                    }).sort((a, b) => (a.dist - b.dist) + (a.isBFS ? -0.5 : 0) - (b.isBFS ? -0.5 : 0));
                    let fwMoved = false;
