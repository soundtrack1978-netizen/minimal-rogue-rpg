...
function initMap() {
        ...
        function generateOneScreen(sx, sy, screenType) {
            ...
            if (screenType === 'breaker') {
                ...
                const trappedItems = [
                    ...
                    SYMBOLS.SPEED, SYMBOLS.HEAL_TOME, SYMBOLS.GUARDIAN
                ];
            ...
            // 1部屋あたりの魔導書を最大2個に制限
            const tomeSyms = [SYMBOLS.SPEED, SYMBOLS.CHARM, SYMBOLS.STEALTH, SYMBOLS.HEAL_TOME, SYMBOLS.EXPLOSION, SYMBOLS.GUARDIAN, SYMBOLS.ESCAPE, SYMBOLS.BREAKER_TOME];
            rooms.forEach(room => {
                let tomeCount = 0;
    ...
    // --- 1部屋あたりの魔導書を最大2個に制限 ---
    const tomeSymbols = [SYMBOLS.SPEED, SYMBOLS.CHARM, SYMBOLS.STEALTH, SYMBOLS.HEAL_TOME, SYMBOLS.EXPLOSION, SYMBOLS.GUARDIAN, SYMBOLS.ESCAPE, SYMBOLS.BREAKER_TOME];
    rooms.forEach(room => {
        let tomeCount = 0;
        for (let y = room.y; y < room.y + room.h; y++) {
            for (let x = room.x; x < room.x + room.w; x++) {
...
function drawMenuScreen() {
    ...
    const options = ["1. ITEMS", "2. STATUS", "3. RINGS"];
    ctx.textAlign = 'left';
    options.forEach((opt, i) => {
        ctx.font = '16px Courier New';
        ctx.fillStyle = '#fff';
...
window.addEventListener('keydown', async e => {
    ...
    if (e.key === 'Enter') {
        ...
        if (gameState === 'TITLE') {
            ...
            else if (menuSelection === 2) { gameState = 'RINGS'; ringEquipSelection = 0; }
            SOUNDS.SELECT();
            return;
        } else if (gameState === 'SHOP') {
            const item = shopStock[shopSelection];
