# MINIMAL ROGUE - Project Progress (2026-03-24 Update)
...
- **シンボル構成**: プレイヤー(`＠`), 壁(`█`), 床(`·`), 敵(`E`), 穴(`◯`), カギ(`k`), 扉(`☒`), 設置ブロック(`□`), 聖なる剣(`†`), 聖なる鎧(`▼`), 加速の魔導書(`▤`), 魅了の魔導書(`▤`), オーク(`O`)。
- **動的なUI**:
...
- **落下アニメーション**: フロア移動・コンテニュー時に「FLOOR X」表示＋キャラ落下演出。
...
- **オートセーブ**: Sタイル廃止。フロア開始時に自動保存。死亡後コンテニューで同フロアから再スタート（ステージ・アイテムはフロア開始時点に戻る）。
- **指輪の重複購入防止**: 商人から同じ指輪を2つ以上買えないよう修正。

### 3. 敵キャラクター & AI
- **オーク (O)**: 地下10階から出現する強敵。
    - **力の溜め (Charge-up)**: 攻撃前に大きく身を引き、小刻みに震えながら力を蓄えるダイナミックな予兆演出。
    - **おしのけ移動 (Shove)**: 進路を他の敵に塞がれた際、その敵と場所を入れ替わって強引に前進する。
    - **無限の突き飛ばし**: 殴られると壁や穴にぶつかるまで吹き飛ばされる。
    - **衝突ダメージ (BUMP!)**: 吹き飛ばされた主人公が他の敵に当たると「人間弾丸」に。
    - **100階魔導書ドロップ**: 100階でオークを倒すと20%の確率でランダムな魔導書をドロップ（誰が倒しても発動）。
- **ミミック部屋（超低確率: 0.2%）**: 15階以降、ごく稀に1部屋の全ての敵がミミックに入れ替わる特殊イベント。
- **既存の敵**: 通常敵(`E`), ゴールデン・エネミー, ウィル・オ・ウィスプ(`※`), FROST, BLAZE, TURRET, BREAKER, LAYER。

### 4. バランス調整
- **スタミナ係数下限**: 0.3 → 0.1（スタミナ切れ時のダメージをより低く）。
- **フロア移動時HP全回復**: 階段を降りるとHPが全回復。

### 5. BGM システム
- **Web Audio API + mp3**: 3曲をランダムにローテーション（直前と同じ曲は流れない）。
- **フェードアウト**: 曲の再生開始から120秒後に30秒かけてゆっくりフェードアウト。
- **フロア間引き継ぎ**: 再生中はそのまま次フロアへ引き継ぐ。無音状態でフロア移動した時だけ新曲を開始。
- **100階**: 常に無音（ボス戦演出）。

## テクニカル
- **Sound**: 全て Web Audio API による動的生成（効果音）＋ mp3（BGM）。
- **UI Architecture**: HTML要素を最小限に抑え、Canvas上で全てを描画。CSSによるスムーズなバー・アニメーション。
- **Logic**: 非同期処理(`async/await`)を多用した、イベント駆動型のターンベースシステム。
- **テストスイート**: `test.js`（Node.js）で47件の自動テスト。ゲームロジック（HP計算、ダメージ計算、敵HP等）を検証。
- **デスクトップアプリ**: `~/Desktop/MinimalRogue.app`（osacompileで生成、ワンクリック起動）。
